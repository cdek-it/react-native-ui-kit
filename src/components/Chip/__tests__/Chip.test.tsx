...
describe('Chip component tests', () => {
  const snapshotCases: Array<[string, ChipProps]> = [
    ['onRemove, icon: empty, disabled: empty/false', { onRemove: jest.fn(), label: 'Chip' }],
    [
    ...
    ['onRemove: empty, icon: empty, disabled: true', { disabled: true, label: 'Chip' }],

    ['onRemove: empty, icon: IconUser, disabled: empty/false', { Icon: IconUser, label: 'Chip' }],
    [
  ...
  test('should handle tap on chip', async () => {
    ...
    const { getByTestId } = render(<Chip label='Chip' onPress={mockedOnTapChip} />)
  ...
  test('should handle tap on remove button', async () => {
    ...
    const { getByTestId } = render(<Chip label='Chip' onRemove={mockedOnClose} />)
