...
компонентов для `ThemeType.components`. `fonts.json` содержит общие для цветовых
схем токены из `primitive.fonts`, но пока не подключён к теме и не
экспортируется из библиотеки. Всё генерируется из
