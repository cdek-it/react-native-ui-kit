...
4. Закоммитить входные и сгенерированные токены вместе.

`tokens:generate` сначала собирает и валидирует весь результат в памяти. При
ошибке файлы не изменяются. После успешной проверки команда удаляет лишние JSON
и записывает отформатированный результат.
...
`yarn tokens:check` ничего не изменяет и отдельно показывает отсутствующие,
изменённые и лишние JSON. `yarn tokens:typecheck` проверяет TypeScript-код
генератора. Обе команды запускаются при сборке и в CI. Если проверка файлов не
прошла, нужно выполнить `yarn tokens:generate` и проверить diff.
